<!--This document is best viewed in Preview mode for readability and document linking-->

# Ember Media Manager - Documentation Index

| | Document Info |
|---------------|---|
| **Version** | 2.1 |
| **Created** | December 31, 2025 |
| **Updated** | January 8, 2026 |
| **Author** | Eric H. Anderson |
| **Purpose** | Master index for all project documentation |

---

## Overview

Welcome! This is your one-stop navigation hub for all Ember Media Manager documentation.

Whether you're looking for performance analysis, scraping process details, or release notes — you'll find it here. Documents are organized by purpose to help you quickly find what you need.

**New here?** Start with the [Project Index](ProjectIndex.md) for a tour of all 24 projects in the solution.

**Contributing docs?** See [Documentation Standards](process-docs/DocumentationStandardsProcess.md) for standards and templates.

---

## Table of Contents

- [Analysis Documents](#analysis-documents)
- [Archived Documents](#archived-documents)
- [Improvements Documents](#improvements-documents)
- [Performance Data](#performance-data)
- [Process Documents](#process-documents)
- [Project Documents](#project-documents)
- [Release Notes](#release-notes)

---

## [↑](#table-of-contents) Analysis Documents

| Document | Description |
|----------|-------------|
| [AsyncImageDownloadAnalysis.md](analysis-docs/AsyncImageDownloadAnalysis.md) | Async image download regression analysis |
| [ImageThumbnailAnalysis.md](analysis-docs/ImageThumbnailAnalysis.md) | Thumbnail download efficiency analysis (resolved) |
| [IMDBScraperAnalysis.md](analysis-docs/IMDBScraperAnalysis.md) | IMDB scraper architecture deep-dive |
| [PerformanceAnalysis.md](analysis-docs/PerformanceAnalysis.md) | Initial performance analysis and optimization opportunities |
| [SolutionCleanupAnalysis.md](analysis-docs/SolutionCleanupAnalysis.md) | Code cleanup opportunities and technical debt |

---

## [↑](#table-of-contents) Archived Documents

| Document | Description |
|----------|-------------|
| [BulkScrapingDocumentation.md](archived-docs/BulkScrapingDocumentation.md) | Bulk scraping (archived - see ScrapingProcessMovies) |
| [FileCleanUpPlan.md](archived-docs/FileCleanUpPlan.md) | Solution cleanup tracking |
| [NuGetPackageUpdatePlan.md](archived-docs/NuGetPackageUpdatePlan.md) | System.* package updates (Dec 2025) |
| [PackageUpdatePlan.md](archived-docs/PackageUpdatePlan.md) | NuGet package update tracking (Dec 2025) |

---

## [↑](#table-of-contents) Improvements Documents

| Document | Description |
|----------|-------------|
| [AddonRemovalPlan.md](improvements-docs/AddonRemovalPlan.md) | Legacy scraper addon removal (complete) |
| [_BL-TEMPLATE.md](improvements-docs/backlog/_BL-TEMPLATE.md) | Template for creating backlog documents |
| [FutureEnhancements.md](improvements-docs/FutureEnhancements.md) | Living backlog of future opportunities |
| [IntegratedAddonRemoval.md](improvements-docs/IntegratedAddonRemoval.md) | NMT/Boxee/Kodi Addons removal plan |
| [NfoFileImprovements.md](improvements-docs/NfoFileImprovements.md) | Planned NFO processing improvements |
| [PerformanceImprovements-Phase1.md](improvements-docs/PerformanceImprovements-Phase1.md) | Phase 1: Infrastructure & parallel image downloads |
| [PerformanceImprovements-Phase2.md](improvements-docs/PerformanceImprovements-Phase2.md) | Phase 2: TV Show async support |
| [PerformanceImprovements-Phase2-2.md](improvements-docs/PerformanceImprovements-Phase2-2.md) | Phase 2-2: Parallel movie scraping |
| [PerformanceImprovements-Phase3.md](improvements-docs/PerformanceImprovements-Phase3.md) | Phase 3: TV Show parallel scraping |
| [PerformanceImprovements-Phase4.md](improvements-docs/PerformanceImprovements-Phase4.md) | Phase 4: IMDB optimization & tracking |
| [PerformanceImprovements-Summary.md](improvements-docs/PerformanceImprovements-Summary.md) | Summary of all performance phases completed |

---

## [↑](#table-of-contents) Performance Data

| Document | Description |
|----------|-------------|
| [IMDB-Baseline-2025-12-31.md](performance-data/IMDB-Baseline-2025-12-31.md) | IMDB scraper baseline measurements |
| [PerformanceData-Movies.md](performance-data/PerformanceData-Movies.md) | Movie scraping baseline metrics |
| [PerformanceData-Tv.md](performance-data/PerformanceData-Tv.md) | TV scraping baseline metrics |

---

## [↑](#table-of-contents) Process Documents

| Document | Description |
|----------|-------------|
| [BuildProcess.md](process-docs/BuildProcess.md) | Build configuration and output |
| [CertificationMappingProcess.md](process-docs/CertificationMappingProcess.md) | Certification/rating mapping system |
| [CountryMappingProcess.md](process-docs/CountryMappingProcess.md) | Country mapping system |
| [DocumentationStandardsProcess.md](process-docs/DocumentationStandardsProcess.md) | Documentation standards and guidelines |
| [FFmpegProcess.md](process-docs/FFmpegProcess.md) | FFmpeg integration and update guide |
| [GenreMappingProcess.md](process-docs/GenreMappingProcess.md) | Genre mapping system |
| [ImageSelectionProcess.md](process-docs/ImageSelectionProcess.md) | Image selection dialog and download pipeline |
| [LanguageProcess.md](process-docs/LanguageProcess.md) | Language handling and mapping |
| [MediaInfoMappingProcess.md](process-docs/MediaInfoMappingProcess.md) | MediaInfo.dll integration and update guide |
| [NfoFileProcess.md](process-docs/NfoFileProcess.md) | NFO file processing analysis |
| [ScrapingProcessMovies.md](process-docs/ScrapingProcessMovies.md) | Movie scraping architecture |
| [ScrapingProcessTvShows.md](process-docs/ScrapingProcessTvShows.md) | TV Show scraping architecture |
| [StatusMappingProcess.md](process-docs/StatusMappingProcess.md) | Status mapping system |
| [StudioMappingProcess.md](process-docs/StudioMappingProcess.md) | Studio mapping system |

---

## [↑](#table-of-contents) Project Documents

| Document | Description |
|----------|-------------|
| [CONTRIBUTING.md](../../.github/CONTRIBUTING.md) | Development guide and workflow |
| [ForkChangeLog.md](ForkChangeLog.md) | Complete change history since forking from upstream |
| [ProjectIndex.md](ProjectIndex.md) | Complete index of all 24 projects in the solution |

---

## [↑](#table-of-contents) Release Notes

| Document | Description |
|----------|-------------|
| [ReleaseNotes-v1.12.0.0.md](release-notes/ReleaseNotes-v1.12.0.0.md) | Version 1.12.0.0 - Framework upgrade |
| [ReleaseNotes-v1.12.1.0.md](release-notes/ReleaseNotes-v1.12.1.0.md) | Version 1.12.1.0 - Bug fixes & documentation |

---

*End of file*